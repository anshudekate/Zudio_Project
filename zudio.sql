...
select *,sum(sales_profit),over (partition by month)  from zudio_category as c
inner join cust_detail as d
on c.customer_id = d.customer_id


create view cloth_table as 
select customer_name ,category , clothing_type , quantity, month, price ,sum(price) over(partition by clothing_type)
from cust_detail as cu
inner join zudio_category as ca
